...
#define OUTPUT_DATA_DIR std::string("../../data/hubbard/")
#endif

const std::string PHASE_FOLDER = std::string(OUTPUT_DATA_DIR) + "phases/";
...
void PhaseHandler::execute(mrock::utility::InputFileReader& input) const {
    ...
    for (int i = 0; i < NUMBER_OF_PARAMETERS; ++i) {
        MPI_Allgather(local_data[i].data(), FIRST_IT_STEPS * SECOND_IT_STEPS, _MPI_RETURN_TYPE, recieve_data[i].data(),
                      FIRST_IT_STEPS * SECOND_IT_STEPS, _MPI_RETURN_TYPE, MPI_COMM_WORLD);
    }
#else
    for (int i = 0; i < NUMBER_OF_PARAMETERS; ++i) {
    ...
    if (rank == 0) {
        ...
        std::filesystem::create_directories(PHASE_FOLDER + output_folder);

        mrock::utility::save_data(recieve_data[0], SECOND_IT_STEPS, PHASE_FOLDER + output_folder + "cdw.dat.gz",
                                  comments);
        mrock::utility::save_data(recieve_data[1], SECOND_IT_STEPS, PHASE_FOLDER + output_folder + "afm.dat.gz",
                                  comments);
        mrock::utility::save_data(recieve_data[2], SECOND_IT_STEPS, PHASE_FOLDER + output_folder + "sc.dat.gz",
                                  comments);
        mrock::utility::save_data(recieve_data[3], SECOND_IT_STEPS, PHASE_FOLDER + output_folder + "gamma_sc.dat.gz",
                                  comments);
        mrock::utility::save_data(recieve_data[4], SECOND_IT_STEPS, PHASE_FOLDER + output_folder + "xi_sc.dat.gz",
                                  comments);
        mrock::utility::save_data(recieve_data[5], SECOND_IT_STEPS, PHASE_FOLDER + output_folder + "eta.dat.gz",
                                  comments);
        mrock::utility::save_data(recieve_data[8], SECOND_IT_STEPS, PHASE_FOLDER + output_folder + "ps.dat.gz",
                                  comments);

        std::chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
        std::cout << "Crude computation time = "
    ...
    if (input.getBool("improved_boundaries")) {
        ...
        for (int i = 0; i < NUMBER_OF_GAP_VALUES; ++i) {
            ...
            MPI_Gatherv(local[i].data(), sizes[i], _MPI_RETURN_TYPE, recieve_boundaries[i].data(), all_sizes[i].data(),
                        displacements.data(), _MPI_RETURN_TYPE, 0, MPI_COMM_WORLD);
        }
#else
        for (int i = 0; i < NUMBER_OF_GAP_VALUES; ++i) {
        ...
        if (rank == 0) {
            ...
            comments.push_back(input.getString("second_iterator_type") + "_MIN=" + std::to_string(SECOND_IT_MIN) +
                               ...
                               "_MAX=" + std::to_string(SECOND_IT_MAX));
            ...
            for (int i = 0; i < NUMBER_OF_GAP_VALUES; ++i) {
                ...
                mrock::utility::save_data(buffer, PHASE_FOLDER + output_folder + "boundaries_" + names[i] + ".dat.gz",
                                          comments);
            }
        }
    }
