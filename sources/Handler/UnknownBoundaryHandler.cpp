...
void UnknownBoundaryHandler::execute(mrock::utility::InputFileReader& input) const {
    ...
    Hubbard::Models::ModelParameters modelParameters(
        model_params[0], model_params[1], model_params[2], (FIRST_IT_MAX - FIRST_IT_MIN) / FIRST_IT_STEPS, 0.0,
    ...
    for (int i = 0; i < FIRST_IT_STEPS; ++i) {
        ...
        if (result_lower == result_upper) {
            ...
            std::cout << "Rank #" << rank << ": skipped iteration #" << i << " with runtime "
                      << std::chrono::duration_cast<std::chrono::milliseconds>(end - begin).count() << "[ms]";
            std::cout << " - both are " << (result_lower ? "negative" : "positive") << " - " << modelParameters
                      << std::endl;
        ...
        for (std::size_t iter = 0U; iter < MAX_ITERATIONS && abs(upper - lower) > ERROR_MARGIN; ++iter) {
            ...
            if (iter == MAX_ITERATIONS - 1)
                std::cerr << "Finished loop #" << i
                          << " with the maximum number of iterations. Precision: " << upper - lower << std::endl;
        }
...
#ifndef _NO_MPI
    MPI_Gather(local_data.data(), FIRST_IT_STEPS, MPI_DOUBLE, recieve_data.data(), FIRST_IT_STEPS, MPI_DOUBLE, 0,
               MPI_COMM_WORLD);
#else
    ...
    if (rank == 0) {
        ...
        mrock::utility::save_data(global_iterator_data, recieve_data,
                                  PHASE_FOLDER + output_folder + "unkown_boundary.dat.gz", comments);
    }
}