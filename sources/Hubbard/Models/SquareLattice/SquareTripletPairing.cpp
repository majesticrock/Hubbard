...
void SquareTripletPairing::fillHamiltonian(const NumericalMomentum<2>& k_values) {
    ...
    diagonalBlock(0, 1) =
        DELTA_CDW - DELTA_AFM;  // +((gamma_cdw - this->gamma_afm) * GAMMA + (xi_cdw - this->xi_afm) * XI);
...
SquareTripletPairing::SquareTripletPairing(const ModelParameters& _params) : HubbardCDW(_params) {
    init();
}
