...
const std::string DATA_FILE_NAME{std::string(OUTPUT_DATA_DIR) + std::string("dos_square.bin")};
...
void Square::computeValues() {
    ...
    auto compute_DOS = [](abscissa_t gamma, abscissa_t one_minus_gamma) -> dos_precision {
        ...
        return (LONG_1_PI * LONG_1_PI) *
               boost::math::ellint_1((gamma < 0.25 ? sqrt(1 - gamma * gamma) : sqrt(one_minus_gamma * (1 + gamma))))
                   .convert_to<dos_precision>();
    };

    tanh_sinh_helper<abscissa_t, dos_precision> tsh{0, 2};
    tanh_sinh_helper<abscissa_t, dos_precision>::SaveTo buffer_vectors{&abscissa, &upper_border_to_abscissa, &weights,
                                                                       &values};
...
#pragma omp parallel for reduction(+ : new_integral) schedule(dynamic)
    ...
    std::cout << "Square: Exit after " << tsh.level() << " levels with error = " << abs(0.5 - new_integral)
              << std::endl;
