...
const std::string DATA_FILE_NAME{std::string(OUTPUT_DATA_DIR) + std::string("dos_simple_cubic.bin")};
...
inline _internal_precision derivative_R(const abscissa_t& x) {
    ...
    return static_cast<_internal_precision>(
        (x * x * (1 - boost::math::ellint_1(ALPHA)) + 4 * (boost::math::ellint_2(ALPHA) - 1)) /
        (x * (x + 2) * (x - 2)));
}
...
inline _internal_precision I_1(const abscissa_t& gamma) {
    ...
    _internal_precision ret = static_cast<_internal_precision>(asin(upper_bound) * R(upper_bound + gamma) -
                                                               asin(lower_bound) * R(lower_bound + gamma));

    auto integrand = [&gamma](const abscissa_t& phi) { return asin(phi) * derivative_R(phi + gamma); };

    ret -= static_cast<_internal_precision>(boost::math::quadrature::gauss_kronrod<abscissa_t, 30>::integrate(
        integrand, lower_bound, upper_bound, 10, 1e-12));
    return ret;
}
...
void SimpleCubic::computeValues() {
    ...
    auto compute_DOS = [](abscissa_t gamma, [[maybe_unused]] abscissa_t one_minus_gamma) -> dos_precision {
        return static_cast<dos_precision>(boost::math::pow<3>(LONG_1_PI) *
                                          (I_1(gamma) - I_2(gamma.convert_to<_internal_precision>())));
    };
