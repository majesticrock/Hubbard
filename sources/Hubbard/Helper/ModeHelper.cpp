...
#define COMMUTATOR_DIR "../commutators/hubbard/"
#endif

namespace Hubbard::Helper {
void ModeHelper::checkTermValidity(const mrock::symbolic_operators::WickTerm& term) {
    if (term.delta_momenta.size() > 1)
        throw mrock::symbolic_operators::bad_term_exception("Too many deltas: " + term.delta_momenta.size(), term);
    if (term.delta_momenta.size() == 1) {
        if (term.delta_momenta[0].first.momentum_list.size() != 1)
            throw mrock::symbolic_operators::bad_term_exception(
                "First delta list is not of size 1: " + term.delta_momenta[0].first.momentum_list.size(), term);
    }

    if (term.coefficients.size() > 1U)
        throw mrock::symbolic_operators::bad_term_exception(
            "Undefined number of coefficients: " + std::to_string(term.coefficients.size()), term);
    if (term.operators.size() > 2U)
        throw mrock::symbolic_operators::bad_term_exception(
            "There are more than 2 WickOperators: " + term.operators.size(), term);
    if (term.sums.momenta.size() > 0U) {
        if (!term.has_single_coefficient())
            throw mrock::symbolic_operators::bad_term_exception("Too many sums: " + term.sums.momenta.size(), term);
        if (term.delta_momenta.empty())
            throw mrock::symbolic_operators::bad_term_exception("There is a summation without delta_kl.", term);
    } else {
        if (term.operators.size() > 2U)
            throw mrock::symbolic_operators::bad_term_exception(
                "A term without a sum can only be bilinear, quartic or an identity.", term);
    }
    if (!(term.coefficients.empty())) {
        // if (term.get_first_coefficient().depends_on_momentum()) {
        //	if (!(term.get_first_coefficient().depends_on('k'))) throw
        // mrock::symbolic_operators::bad_term_exception("Each momentum dependent term should have a k-depedance.",
        // term);
        // }
        if (term.get_first_coefficient().momenta.size() > 1U)
            throw mrock::symbolic_operators::bad_term_exception(
                "There must not be more than 1 momentum in coefficient!", term);
    }
}

ModeHelper::ModeHelper(mrock::utility::InputFileReader& input)
    : number_of_basis_terms{input.getInt("number_of_basis_terms")},
    ...
    wicks.load(std::string(COMMUTATOR_DIR) + subfolder, this->start_basis_at < 0, this->number_of_basis_terms,
               this->start_basis_at);
    try {
