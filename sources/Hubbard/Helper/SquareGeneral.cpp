...
void SquareGeneral::fill_block_M(int i, int j) {
    ...
    if ((broken_symmetry & BrokenSymmetry::ParticleNumber) && !(order & OrderType::SC))
        return;
    if ((broken_symmetry & BrokenSymmetry::Momentum) && !(order & (OrderType::CDW | OrderType::AFM)))
        return;
...
void SquareGeneral::setNewModelParameters(mrock::utility::InputFileReader& input,
