...
void GeneralBasis::create_starting_states() {
    ...
    const global_floating_type norm_constant = this->usingDOS
                                                   ? std::sqrt((2.0 * this->dos_dimension) / Constants::BASIS_SIZE)
                                                   : std::sqrt(1. / ((global_floating_type)Constants::BASIS_SIZE));

    int n_starting_states = 2;  // SC Higgs and Phase
    if (number_of_basis_terms >= 6)
        n_starting_states += 2;  // AFM longitudinal and CDW
    if (number_of_basis_terms >= 10)
        ++n_starting_states;  // AFM transversal
...
void GeneralBasis::printDOSBlocks() const {
    ...
    for (int k = 0; k < Constants::BASIS_SIZE / 2; ++k) {
        ...
        for (int i = 0; i < number_of_basis_terms; ++i) {
            ...
            for (int j = 0; j < number_of_basis_terms; ++j) {
                printM(j + (k + Constants::BASIS_SIZE / 2) * number_of_basis_terms,
                       i + (k + Constants::BASIS_SIZE / 2) * number_of_basis_terms);
            }
            std::cout << std::endl;
        }
        std::cout << std::endl;
