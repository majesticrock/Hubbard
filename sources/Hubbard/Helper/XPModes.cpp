...
void XPModes::create_starting_states() {
...
#else
        this->usingDOS ? std::sqrt((2.0 * this->dos_dimension) / Constants::BASIS_SIZE)
    ...
    for (int i = 0; i < Constants::BASIS_SIZE; ++i) {
        ...
        starting_states[2][1](2 * Constants::BASIS_SIZE + i) =
            (i < Constants::BASIS_SIZE / 2) ? norm_constant : -norm_constant;
