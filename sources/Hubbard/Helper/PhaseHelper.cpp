...
PhaseHelper::PhaseHelper(mrock::utility::InputFileReader& input, int _rank, int _nRanks)
    ...
    modelParameters = ModelParameters(model_params[0], model_params[1], model_params[2],
                                      (FIRST_IT_MAX - FIRST_IT_MIN) / FIRST_IT_STEPS,
                                      (SECOND_IT_MAX - SECOND_IT_MIN) / SECOND_IT_STEPS,
                                      input.getString("global_iterator_type"), input.getString("second_iterator_type"));
}

std::unique_ptr<BaseModel<global_floating_type>> PhaseHelper::getModelType(
    ...
    if (startingValues.has_value()) {
        switch (_internal_lattice_type) {
            ...
            case 129:
                throw std::runtime_error("1D Chain: To be implemented!");
            case 130:
                return std::make_unique<DOSModels::BroydenDOS<DensityOfStates::Square>>(mp, startingValues.value(),
                                                                                        10U);
            case 131:
                return std::make_unique<DOSModels::BroydenDOS<DensityOfStates::SimpleCubic>>(mp, startingValues.value(),
                                                                                             10U);
            default:
                throw std::runtime_error("_internal_lattice_type not properly set " +
                                         to_string(_internal_lattice_type));
        }
    } else {
        switch (_internal_lattice_type) {
            case 1:
...
ModelAttributes<global_floating_type> PhaseHelper::computeDataPoint(
    const ModelParameters& mp,
    std::optional<ModelAttributes<global_floating_type>> startingValues /*= std::nullopt*/) {
...
ModelAttributes<global_floating_type> PhaseHelper::computeDataPoint_No_AFM_CDW_Fix(
    const ModelParameters& mp,
...
void PhaseHelper::findSingleBoundary(const std::vector<data_vector>& origin,
                                     data_vector& recieve_data,
                                     int value_index) {
    ...
    while (plaqs.size() > 0U && plaqs.begin()->size() > 5e-5) {
        std::cout << "Plaquette size: " << plaqs.begin()->size() << "\t"
                  << "Current number of Plaquettes: " << plaqs.size() << std::endl;
...
#pragma omp parallel for
        ...
        if (totalSize > (8 * this->FIRST_IT_STEPS > 200 ? 8U * static_cast<std::size_t>(this->FIRST_IT_STEPS)
                                                        : std::size_t{200U})) {
...
void PhaseHelper::coexistence_AFM_CDW(std::vector<data_vector>& recieve_data) {
    ...
    auto base_V = [](coefficient_type U) -> coefficient_type { return 0.25 * U; };
    ...
    for (int i = 0; i < FIRST_IT_STEPS; ++i) {
        ...
        base_gap[0] = ONE_OVER_SQRT_2 * std::sqrt(base_gap[0] * base_gap[0] + base_gap[1] * base_gap[1]);
        base_gap[1] = 0;
