...
global_floating_type TermOnSquare::getExpectationValue(const mrock::symbolic_operators::WickOperator& op,
                                                       const Eigen::Vector2i& momentum_value) const {
    ...
    if (op.type == mrock::symbolic_operators::OperatorType::CDW ||
        ...
        if (jt == wick_spin_offset.end())
            throw std::runtime_error("Something went wrong while looking up the spin indizes.");
        index += jt->second;
    }
...
Eigen::Vector2i TermOnSquare::compute_momentum_list(const mrock::symbolic_operators::MomentumList& momentum,
                                                    const Eigen::Vector2i& k,
                                                    const Eigen::Vector2i& l) const {
    if (momentum.empty()) {
...
Eigen::Vector2i TermOnSquare::compute_momentum_no_q(const mrock::symbolic_operators::Momentum& momentum,
                                                    const Eigen::Vector2i& k,
                                                    const Eigen::Vector2i& l) const {
...
global_floating_type TermOnSquare::compute_sum(const mrock::symbolic_operators::WickTerm& term,
                                               const Eigen::Vector2i& k,
                                               const Eigen::Vector2i& l) const {
...
global_floating_type TermOnSquare::computeTerm(const mrock::symbolic_operators::WickTerm& term,
                                               const int k,
                                               const int l) const {
    const OrderType model_order = this->model->get_order();
    if (term.includes_type(mrock::symbolic_operators::OperatorType::CDW) &&
        !(model_order & (OrderType::CDW | OrderType::AFM)))
        return global_floating_type{};
    if (term.includes_type(mrock::symbolic_operators::OperatorType::SC) && !(model_order & OrderType::SC))
        return global_floating_type{};
