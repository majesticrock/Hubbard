...
void SquareXP::fill_block_M(int i, int j) {
    const int sum_limit =
        std::find(cdw_basis_positions.begin(), cdw_basis_positions.end(), i) == cdw_basis_positions.end()
            ? Constants::BASIS_SIZE
            : Constants::BASIS_SIZE / 2;
    const int inner_sum_limit =
        std::find(cdw_basis_positions.begin(), cdw_basis_positions.end(), j) == cdw_basis_positions.end()
            ? Constants::BASIS_SIZE
            : Constants::BASIS_SIZE / 2;
        ...
        for (int k = 0; k < sum_limit; ++k) {
            if (term.delta_momenta.size() > 0U) {
                ...
                if (term.delta_momenta[0].second.momentum_list.size() == 2U) {
                    l = add_index_momenta(l, this->mode_momentum,
                                          -term.delta_momenta[0].second.momentum_list[1].factor);
                }
                ...
                if (i < hermitian_size) {
                    ...
                    K_minus(antihermitian_offsets[i - hermitian_size] + k,
                            antihermitian_offsets[j - hermitian_size] + l) += computeRealTerm(term, k, l);
                }
            } else {
...
void SquareXP::setNewModelParameters(mrock::utility::InputFileReader& input,
                                     const Models::ModelParameters& modelParameters) {
