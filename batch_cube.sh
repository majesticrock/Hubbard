...
for NEW_VALUE in "${NEW_VALUES[@]}"; do
  ...
  sed -i "s|#SBATCH --output=/home/althueser/phd/cpp/Hubbard/modes_output.txt|#SBATCH --output=/home/althueser/phd/cpp/Hubbard/cube_output_$NEW_NAME.txt|" cube_auto_generated/$NEW_NAME.slurm
  sed -i "s|mpirun ./build/cascadelake/Hubbard params/cluster_cube.config|mpirun ./build/cascadelake/Hubbard cube_auto_generated/$NEW_NAME.config|" cube_auto_generated/$NEW_NAME.slurm
